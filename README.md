...
###### 💻Then go to ansible host path and create one group " [docker] " then add your docker-server private-IP in that group
...
- go to your git-hub account and go to your source-code repo in source-code repo go to **( settings )** then click on **( webhooks )** then click on add webhook then in **payload url** type your **( jenkins url )** ( Ex.- http://<public ip of jenkins server>:8080/github-webhook/ ) ...then in **( content type )** option select **application json** option...

- then in **( secret )** option you have to provide your jenkins secret token so for that go to your jenkins dashboard and then click on your leftside profile-icon option then click on **( security )** option then find **( API Token )** option then click on *add token* after that generate the token then copy that token and then click apply button and save it and paste your copied token in github-webhook **(secreat )** option and then finally click on **addwebhook** option...
